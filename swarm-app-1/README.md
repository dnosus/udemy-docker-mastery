...
docker service create --name db --network backend --env POSTGRES_HOST_AUTH_METHOD=trust --mount type=volume,source=db-data,target=/var/lib/postgresql/data postgres:9.4
...
docker service create --name result --network backend --publish 5001:80 bretfisher/examplevotingapp_result